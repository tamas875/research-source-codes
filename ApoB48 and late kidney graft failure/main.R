...
library(rstudioapi)
current_dir = gsub("main.R", "", getSourceEditorContext()$path)
data_locations = read.csv(paste0(current_dir, "data_location.csv"))

library(foreign)
d.aiko <- read.spss(data_locations[1,], 
                    use.value.labels = TRUE, 
                    to.data.frame = TRUE)
d.apob48 <- read.csv(data_locations[2,])

colnames(d.apob48) <- c('ki_num', 'b_apob48_fu1')
d.aiko <- merge(d.aiko, d.apob48, by = 'ki_num')

d.aiko <- d.aiko[which(is.na(d.aiko['CKD_EPI']) == FALSE 
                       & is.na(d.aiko['graft_failure_2012']) == FALSE 
                       & is.na(d.aiko['b_apob48_fu1']) == FALSE),]
rm(d.apob48)
...
if (FALSE) {
  ...
  tertiles.apob48 <- lapply(split.data, function(group) {
    group$tertiles <- cut(group$b_apob48_fu1, 
                          breaks = quantile(group$b_apob48_fu1, 
                                            probs = c(0, 1/3, 2/3, 1)), 
                          labels = c("T1", "T2", "T3"))
    return(group)
...
if (TRUE) {
  d.aiko$tertiles <- cut(d.aiko$b_apob48_fu1, 
                         breaks = c(-Inf, quantile(d.aiko$b_apob48_fu1, probs = 1/3), 
                                    quantile(d.aiko$b_apob48_fu1, 
                                             probs = 2/3), Inf), 
                         labels = c("Low tertile", "Medium tertile", "High tertile"))
}
...
d.aiko$hba1c_strat <- cut(d.aiko$per_hba1, c(-Inf, 
                                             median(d.aiko$per_hba1, 
                                                    na.rm = TRUE), Inf),
                          labels = c(paste0('< ', 
                                            round(median(d.aiko$per_hba1, 
                                                         na.rm = TRUE), 1)), 
                                     paste0('> ', 
                                            round(median(d.aiko$per_hba1, 
                                                         na.rm = TRUE), 1))))
...
baseline.vars <- c('age_pat', 'sexe_pat', 'tijd_bas',
                   'con_krea', 'exc_pro', 'CKD_EPI', 
                   'tijd_dia', 'num_mmto',  
                   'bmi_poli', 'gem_tail', 'di_sig', 'con_gluc', 
                   'gem_sbp', 'gem_dbp',
                   'con_crp', 'diabetes_fu1', 'con_insu', 
                   'per_hba1', 'iri_homa',
                   'statin_use_fu1', 'con_tc', 'con_tg', 
                   'con_hdlc', 'con_apoa', 'con_apob', 
...
surv.obj <- Surv(d.aiko$followup_gf_dood_2012, 
                 as.numeric(d.aiko$graft_failure_2012))
...
cox.summary <- CoxSummary(models = cox.list, 
                          var = 'c_apob48_log2', 
                          summ = TRUE, 
                          styled = FALSE)
cox.summary.s <- CoxSummary(models = cox.list, 
                            var = 'c_apob48_log2', 
...
pdf(paste0(current_dir, 'figures/figure_3.pdf'), width = 8.5, height = 5.5)
print(forest.plot)
dev.off()
...
cox.apob.summary <- CoxSummary(models = list(cox.apob.1), 
                               var = 'con_apob', 
                               model.names = c('con_apob'))
...
DescribeData(variables = baseline.vars, 
             normal = c(), 
             group = 'tertiles', 
             df = d.aiko)

cox.cvd.1 <- cox.1 <- coxph(Surv(d.aiko$T_CVD, as.numeric(d.aiko$di_CVD)) ~ 
                              c_apob48_log2, data = d.aiko)
summary(cox.cvd.1)

aiko.raw <- read.spss(data_locations[1,], 
                      use.value.labels = TRUE, 
                      to.data.frame = TRUE)
# Use the NITRA registry to look up which KTR used which induction therapy
nitra.raw = read.spss(data_locations[3,], 
                      use.value.labels = TRUE, 
                      to.data.frame = TRUE)
...
cox.suppl.7 <- coxph(surv.obj ~ c_apob48_tg_log2 + sexe_pat + age_pat + 
               ...
               data = d.aiko)
cox.suppl.list <- list(cox.suppl.1, 
                       cox.suppl.2, 
                       cox.suppl.3, 
                       cox.suppl.4, 
                       cox.suppl.5, 
...
cox.suppl.summary <- CoxSummary(models = cox.suppl.list, 
                                var = 'c_apob48_tg_log2', 
                                summ = TRUE, 
                                styled = FALSE)

CoxSummary(models = list(cox.suppl.7), 
           var = 'c_apob48_tg_log2', 
           summ = TRUE, 
           styled = FALSE)
...
#Plot HR and export to PDF file
pdf(paste0(current_dir, "figures/suppl_fig_2.pdf"), width = 7.5, height = 6)
...
summary(glm((d.aiko$stat_pat_f8_2012 == 'overleden') 
            ~ d.aiko$c_apob48_log2, family = 'binomial'))
round(exp(coef(glm((d.aiko$stat_pat_f8_2012 == 'overleden') 
                   ~ d.aiko$c_apob48_log2, family = 'binomial'))), 2)
round(exp(confint.default(glm((d.aiko$stat_pat_f8_2012 == 'overleden') 
                              ~ d.aiko$c_apob48_log2, family = 'binomial'))), 2)
...
baseline.vars.statines <- c('age_pat', 'sexe_pat', 
                            'b_apob48_fu1', 'con_tc', 
                            'con_tg', 'con_hdlc', 'con_ldlc',
                            'con_apoa', 'con_apob')
...
write.csv(baseline.table.statines, paste0(current_dir, "tables/table_statins.csv"))
